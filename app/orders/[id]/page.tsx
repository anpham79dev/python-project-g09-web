...
export default function OrderDetailPage() {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-secondary font-mono">Mã SP: {record.productId}</span>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Text className="text-secondary text-xs font-mono">
              Mã hóa đơn: <strong className="text-[#006C49]">{order.code}</strong>
            </Text>
          </div>
        </div>
      ...
      <Card className="border border-[#E5E7EB] shadow-xs rounded-xl overflow-hidden" styles={{ body: { padding: '28px 32px' } }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-[#F8F9FA] rounded-xl border border-[#E5E7EB] mb-6 text-xs">
          ...
          <div>
            ...
            <Tag color={order.paymentMethod === 'QR_TRANSFER' ? 'cyan' : order.paymentMethod === 'CASH' ? 'green' : 'blue'}>
              {order.paymentMethod === 'QR_TRANSFER' ? 'Chuyển khoản QR' : order.paymentMethod === 'CASH' ? 'Tiền mặt' : 'Thẻ POS'}
            </Tag>
          </div>
        </div>
        ...
        {order.note && (
          <div className="mb-6 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-900 flex items-center gap-2">
            ...
            <span><strong>Ghi chú đơn hàng:</strong> {order.note}</span>
          </div>
        )}
        ...
        <div className="mb-6">
          <h4 className="font-bold text-sm text-[#111827] uppercase tracking-wider mb-3">
            Danh sách món đã đặt ({order.items.reduce((s, i) => s + i.quantity, 0)} món)
          </h4>
          <Table
            columns={itemColumns}
        ...
        <div className="flex justify-end">
          <div className="w-full max-w-sm space-y-2 text-sm">
            ...
            <div className="flex justify-between items-baseline pt-3 border-t border-[#E5E7EB]">
              <span className="font-bold text-base text-[#111827]">TỔNG THANH TOÁN:</span>
      ...
      <div id="order-detail-printable-receipt" className="hidden font-mono text-black">
        ...
        <div className="text-[10px] space-y-0.5 pb-2 border-b border-dashed border-black">
          <div className="flex justify-between">
            <span>Thời gian:</span>
            <span>{dateObj.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })} - {dateObj.toLocaleDateString('vi-VN')}</span>
          </div>
          <div className="flex justify-between">
            <span>Thu ngân:</span>
          ...
          <div className="flex justify-between">
            ...
            <span>{order.customerName || 'Khách vãng lai'} {order.customerPhone ? `(${order.customerPhone})` : ''}</span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="text-center pt-3 text-[10px] space-y-0.5">
          ...
          <p className="text-[8px] text-gray-500 mt-1">Hóa đơn điện tử lưu trữ trên hệ thống</p>
        </div>
      </div>

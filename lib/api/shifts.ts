...
const getStoredShifts = (): WorkShift[] =>
  getStoredList(LS_KEYS.SHIFTS, INITIAL_SHIFTS);
const saveStoredShifts = (shifts: WorkShift[]) =>
  saveStoredList(LS_KEYS.SHIFTS, shifts);
...
export const closeCurrentShift = async (data: {
  actualCash: number;
  note?: string;
}): Promise<WorkShift> => {
  if (isMockMode()) {
...
export const getShifts = async (params?: {
  ...
  if (isMockMode()) {
    ...
    if (params?.staffId)
      list = list.filter((s) => s.staffId === params.staffId);
    if (params?.branchId && params.branchId !== "ALL") {
      list = list.filter((s) => !s.branchId || s.branchId === params.branchId);
    }
